...
- `tokens::LimitTier` is the only attention scale: calm (white) below 75%, elevated (amber) to 89%, critical (bold red) from 90%, applied solely to capped values (5h/weekly/scoped percentages, the `ex:` credit, the context percent on both surfaces). Costs, the reset countdown, cache and token counts are neutral **by design**: the old `$10`/`$5` gradient ceilings kept `today:`/`win:` permanently red, and the sub-30-minute bold-red countdown sat beside `usage:76%` so the reader could not tell which token was the alarm. The five-hour figure takes `max(level tier, five_hour_pace_tier)`; the pace tier projects exhaustion before the reset once at least 60 minutes of the window have elapsed and caps at elevated. Semantic RGB values mirror Claude Code's dark theme (`warning` 255,193,7; `error` 255,107,128; `success` 78,186,101; `inactive` 153,153,153). Claude Code's Ink fork renders every statusline line inside `<Text dimColor>`, which it maps to `theme.inactive` rather than SGR 2, so uncolored runs turn gray and colored runs pass through verbatim
